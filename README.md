...
> **v0.9.9 (2026-05-10)** — Current GitHub Release. v0.9.9 closes the 8 items in [`.local/feedbacks/feedback_for_v0.9.7.md`](.local/feedbacks/feedback_for_v0.9.7.md) (F1 stdout silence-timer + F2 `popola status` `pid_alive` field + F3 dispatch-time CLI footer & worker idle hint + F4 `_ERROR_CATALOG` GitHub-App branch-not-found regex + F5 + U1 `account_class` pre-flight gate + F6 `_run_subprocess` Popen+setsid signal forwarder + new `popola cloud worker stop` verb + U2 `popola init` 0o600 fallback file at `~/.popola/cursor_api_key.env`). Wave Spike-0 produced [`SCHEMA_INVESTIGATION.md`](.local/.agent/active/v0.9.9-worker-observability/SCHEMA_INVESTIGATION.md) (BRANCH_B verdict): Cursor REST has no documented schema for personal-key + self-hosted-worker dispatch with Dashboard visibility — the v0.9.9 pre-flight gate ships the original Option-A loud-fail per Q-V099-2 with hint text pointing at `popola cloud worker handoff`, `popola dispatch --cli=cursor`, and the My Machines `@Cursor worker=<name>` chat trigger. **GitHub Release-only** (PyPI publish still deferred per `BL-v0.9.x-PyPI`) — install v0.9.9 via `./install.sh install` (canonical), `./install.sh install --ref=v0.9.9` (pinned), `./install.sh install --with-credentials` (canonical + keyring extra), or `pip install git+https://github.com/YoRHa-Agents/PopolaLoom@v0.9.9` (manual fallback) — see [`RELEASE_NOTES.md`](RELEASE_NOTES.md). Upgrades from v0.7.x still walk through [`docs/MIGRATION_v07_to_v09.md`](docs/MIGRATION_v07_to_v09.md).
...
```bash
# v0.9.9 install — GitHub Release path remains canonical while PyPI promotion is deferred.
./install.sh install                 # canonical (default --from=git, tracks main)
./install.sh install --ref=v0.9.9    # reproducible tag-pinned install
...
**v0.9.9 — current release (2026-05-10).** v0.9.9 is a strictly additive patch on the v0.9.0 GA SemVer contract in [`docs/API_STABILITY.md`](docs/API_STABILITY.md): closes the 8 items in [`.local/feedbacks/feedback_for_v0.9.7.md`](.local/feedbacks/feedback_for_v0.9.7.md) — F1 stdout silence-timer + F2 `popola status` `pid_alive` field + F3 dispatch-time CLI footer & worker idle hint + F4 `_ERROR_CATALOG` GitHub-App branch-not-found regex + F5 + U1 `account_class` pre-flight gate + F6 `_run_subprocess` Popen+setsid signal forwarder + new `popola cloud worker stop` verb + U2 `popola init` 0o600 fallback file at `~/.popola/cursor_api_key.env`. Wave Spike-0 [`SCHEMA_INVESTIGATION.md`](.local/.agent/active/v0.9.9-worker-observability/SCHEMA_INVESTIGATION.md) BRANCH_B verdict (high confidence): Cursor REST has no documented schema for personal-key + self-hosted-worker dispatch with Dashboard visibility, so the pre-flight gate ships the original Option-A loud-fail per Q-V099-2. Every v0.9.0 / v0.9.1 / v0.9.2 / v0.9.3 / v0.9.4 / v0.9.5 / v0.9.6 / v0.9.7 / v0.9.8 stable CLI verb, daemon RPC route, public Python API, and Skill front-matter key remains intact. v0.7.x → v0.9.0 migration remains [`docs/MIGRATION_v07_to_v09.md`](docs/MIGRATION_v07_to_v09.md). **For v0.9.9**: install via `./install.sh install` (canonical), `./install.sh install --ref=v0.9.9` (pinned), `./install.sh install --with-credentials` (canonical + keyring extra), or `pip install git+https://github.com/YoRHa-Agents/PopolaLoom@v0.9.9` (manual fallback). PyPI publish remains deferred to a v0.9.x patch per Q-D-5 偏离默认 (see `BL-v0.9.x-PyPI` in TRACKER); pass `--from=pypi --version=0.9.x` once that patch lands.
...
### v0.9.9 — canonical install (install.sh default --from=git since v0.9.6; Q-D-5 偏离默认 carries forward)

> **v0.9.9 recipe** — PyPI publish remains deferred to a v0.9.x patch (Q-D-5 偏离默认; see `BL-v0.9.x-PyPI` in TRACKER); starting in v0.9.6 the official `./install.sh` bootstrap **no longer defaults to PyPI** (`--from` defaults to `git`) and the canonical v0.9.9 tag-pinned recipe is `./install.sh install --ref=v0.9.9` (carries forward the v0.9.6 fix for [`./.local/feedbacks/feedback_for_v0.9.4.md`](.local/feedbacks/feedback_for_v0.9.4.md) lines 2-5). A fresh `./install.sh install` works without PyPI on Chinese pip mirrors that don't carry `popolaloom` yet. Both shapes are also documented in [`docs/MIGRATION_v07_to_v09.md`](docs/MIGRATION_v07_to_v09.md#tldr) for v0.7.x upgraders.

```bash
# Canonical v0.9.9 install (default --from=git, tracks main):
./install.sh install

# Canonical tag-pinned v0.9.9 install (recommended for reproducible installs):
./install.sh install --ref=v0.9.9

# Manual fallback — directly via pip (always-works, tag-pinned):
pip install git+https://github.com/YoRHa-Agents/PopolaLoom@v0.9.9
```

### One-line install (v0.8.4+; default --from=git since v0.9.6)

```bash
# v0.9.9 default — tracks latest main from GitHub (no PyPI required)
...
```bash
# v0.9.9 canonical path — install directly from the GitHub Release tag.
...
> Once v0.9.x publishes to PyPI (`BL-v0.9.x-PyPI`), the bare package-name installer path will work directly. **For v0.9.9 users, use `./install.sh install` or the git URL above** — the bare package-name path currently resolves to the previous v0.8.x line.
...
v0.9.9 closes the 8 items in [`.local/feedbacks/feedback_for_v0.9.7.md`](.local/feedbacks/feedback_for_v0.9.7.md) across 4 source-code waves + 1 schema-investigation spike + 1 docs lift:

- **F1 stdout silence-timer + branched `process.note` event** — `Supervisor.spawn` now registers a 30s silence timer (cancelled by the first non-empty stdout line OR by `_wait_and_finalize` exit-before-fire) that emits a single `process.note` event with branched hint text per Q-V099-14: text-mode hint quotes the user's verbatim feedback `cursor-agent 'text' output is buffered until exit; pass --cli-flag output_format=stream-json for live progress`; stream-json mode emits `cursor-agent is working; first stream-json frame not yet emitted (this can take 60s+ for large prompts)`; other CLIs emit a generic note. `popola attach --follow` no longer goes 10 minutes without any signal during long cursor-agent runs.
- **F2 `popola status` `pid_alive` field** — `Popolad.get_status` now probes `os.kill(pid, 0)` when `runtime=local` AND `state=RUNNING` AND `pid is not None`; surfaces `pid_alive` as an additive field in the status summary (absent for completed/cloud per Q-V099-4); emits a daemon-log WARN on drift. Force-finalize sync trigger deferred to `BL-v0.10.0-supervisor-force-finalize`.
- **F3 dispatch-time CLI footer + worker idle hint** — `popola dispatch --cli=cursor` stdout now ends with `view: popola attach <id> --follow (note: Cursor dashboard does not show local subprocess tasks)` per the user's verbatim wording; the footer is gated on `cli == "cursor"` (cursor-cloud and other adapters keep their existing single-line output). `popola cloud worker status` adds an idle-state hint when 0 sessions are claimed.
- **F4 `_ERROR_CATALOG` `integration_github_app_branch_not_found` regex (catalog 16 → 17)** — new 400-status entry catches the misleading `Failed to verify existence of branch 'main' in repository ...` body that Cursor REST returns when the GitHub App is missing on the target org; reuses `GithubAppMissingError` per Q-V099-7; bilingual hint with both `https://cursor.com/integrations/github` + `https://github.com/apps/cursor` URLs + `auto_create_pr=false` workaround. `cli_exit=78` for parity with the 422 sibling.
- **F5 + U1 `account_class` pre-flight gate per Spike-0 BRANCH_B** — Wave Spike-0 produced [`SCHEMA_INVESTIGATION.md`](.local/.agent/active/v0.9.9-worker-observability/SCHEMA_INVESTIGATION.md) (BRANCH_B verdict, high confidence): Cursor REST has no documented schema for personal-key + self-hosted-worker dispatch with Dashboard visibility; the verdict is corroborated by the OpenAPI spec + 4 doc pages (My Machines + Self-Hosted Pool + endpoints + v0). v0.9.9 ships the original Option-A loud-fail per Q-V099-2: NEW `account_class` metadata field on `credentials.toml` (`personal` / `service-account` / `unknown`, default `unknown` for backward compat); NEW `popola auth cursor set --account-class=...` Typer option + `--no-prompt` flag; pre-flight gate in `worker_dispatch_cmd` refuses with `Exit(78)` + bilingual EN+ZH hint pointing at `popola cloud worker handoff`, `popola dispatch --cli=cursor`, and the My Machines `@Cursor worker=<name>` chat trigger when class ∈ {`personal`, `unknown`}.
- **F6 `_run_subprocess` Popen+setsid signal forwarder + new `popola cloud worker stop` verb** — `_run_subprocess` rewritten from `subprocess.run` to `subprocess.Popen(start_new_session=True)` + SIGTERM/SIGINT pgroup-forwarder so killing the Python wrapper no longer leaves orphan Node `agent worker start` children. NEW `popola cloud worker stop --name X | --worker-dir Y --grace N` Typer verb (verb registration order: `debug` → `start` → **`stop`** → `dispatch` → `status` → `handoff`). `--help` documents the no-idle-gate caveat per Q-V099-6 verbatim.
- **U2 `popola init` 0o600 fallback file at `~/.popola/cursor_api_key.env`** — closes the user's verbatim follow-up at `feedback_for_v0.9.7.md:114-116`. When the keyring is unavailable, `popola init --cursor-api-key VAL` and `popola init --cursor-api-key-file <path>` now persist the secret to a 0o600-protected fallback file containing `CURSOR_API_KEY=<value>\n`; the daemon auto-sources it on startup so `popola dispatch` from a fresh shell after `popola init --cursor-api-key VAL` "just works".
- **Strictly additive patch** — every v0.9.0 / v0.9.1 / v0.9.2 / v0.9.3 / v0.9.4 / v0.9.5 / v0.9.6 / v0.9.7 / v0.9.8 stable CLI verb, daemon RPC route, public Python API, and Skill front-matter key remains intact. v0.10.0 backlog rows generated: `BL-v0.10.0-cursor-personal-key-worker-schema` (depends on Cursor backend cooperation; drafted upstream issue text in the SCHEMA_INVESTIGATION.md), `BL-v0.10.0-supervisor-force-finalize`, `BL-v0.10.0-cursor-cloud-rest-smoke`, `BL-v0.10.0-init-no-cursor-key-flag`, `BL-v0.10.0-init-validate-cursor-key`, `BL-v0.10.0-cursor-output-format-default-flip`. Full details: [`CHANGELOG.md`](CHANGELOG.md) and [`RELEASE_NOTES.md`](RELEASE_NOTES.md).
