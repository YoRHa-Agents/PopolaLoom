...
> **v0.9.4 (2026-05-10)** — Current GitHub Release. v0.9.4 is a strictly additive CI hotfix on top of v0.9.3: it keeps workspace-aware self-hosted worker reuse, direct `popola cloud worker dispatch`, and Cursor Cloud private-worker routing extras, and fixes optional cloud workflow validation so Actions stay green when `CURSOR_API_KEY` is absent. Upgrades from v0.7.x still walk through [`docs/MIGRATION_v07_to_v09.md`](docs/MIGRATION_v07_to_v09.md). **GitHub Release-only** — install v0.9.4 via `pip install git+https://github.com/YoRHa-Agents/PopolaLoom@v0.9.4` (Q-D-5 偏离默认 carries forward: PyPI deferred to v0.9.x; see `BL-v0.9.x-PyPI` in TRACKER and [`RELEASE_NOTES.md`](RELEASE_NOTES.md)). The `./install.sh` bootstrap defaults to PyPI and tracks the **latest published stable** (currently v0.8.x until the v0.9.x PyPI patch lands); use `./install.sh install --from=git` to install latest `main`.
