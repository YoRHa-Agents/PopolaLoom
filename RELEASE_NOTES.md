...
> **G3 verification oracle replaced (Cursor REST `is_in_use=null` empirical regression).** The v1.5.0 oracle "self-hosted-worker dispatch routed correctly when `worker.is_in_use=true active_bc_id=<bc>`" is no longer reliable — `GET /v0/private-workers` clears `is_in_use` the moment the agent enters terminal phase (or within ~30 s of inactivity), even while the worker process still holds the repo lock. The recommended G3 oracle is now **`agent.target.machine_name`** from `GET /v1/agents/<bc-id>` (durable for the agent's lifetime) **PLUS** the Prometheus metric `cursor_self_hosted_worker_last_activity_unix_seconds` from the worker's own `--management-addr` `/metrics`. Documented in both SKILL.md mirrors.
...
| G3 oracle (worker routing) | `agent.target.machine_name` + worker `/metrics` `cursor_self_hosted_worker_last_activity_unix_seconds` (replaces flaky `is_in_use`). |
